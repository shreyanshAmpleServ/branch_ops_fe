...
import type { PurchaseQuotation } from '../api/usePurchaseQuotations';
...
interface PurchaseQuotationReceiptViewProps {
  quotation: PurchaseQuotation;
  onBack: () => void;
}

export const PurchaseQuotationReceiptView: React.FC<PurchaseQuotationReceiptViewProps> = ({ quotation, onBack }) => {
  ...
  const handleDownloadPDF = () => {
    ...
    const opt = {
      ...
      filename: `Purchase_Quotation_${quotation.QuotCode || quotation.ID}.pdf`,
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Purchase Quotation");
    XLSX.writeFile(wb, `Purchase_Quotation_${quotation.QuotCode || quotation.ID}.xlsx`);
  };

  const subtotal = quotation.items?.reduce((sum, item) => sum + (Number(item.Quantity || 0) * Number(item.UnitPrice || 0)), 0) || 0;
  ...
  const amountInWords = "Tanzanian Shillings " + Number(quotation.DocTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + " Only";

  const aprStatus = quotation.AprStatus || 'P';
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-xs font-bold text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 px-2 py-0.5 rounded-md border border-teal-200 dark:border-teal-800">
                {quotation.QuotCode || `ID #${quotation.ID}`}
              </span>
            </div>
            <p className="text-xs text-gray-500 dark:text-gray-400 hidden sm:block">
              Official Purchase Quotation Document View
            </p>
          </div>
        </div>
...
>
          ...
          <div className="w-full bg-gradient-to-r from-[#005f73] via-[#0A9396] to-[#94D2BD] text-white px-10 py-6 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <h1 className="text-2xl font-extrabold tracking-tight uppercase text-white">PURCHASE QUOTATION</h1>
            </div>
          ...
          <div className="p-10 space-y-8">
              ...
              <div className="bg-slate-50 rounded-2xl border border-slate-200/80 p-5 shadow-sm">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                    ...
                    <span className="font-bold text-[#005f73] font-mono text-sm">{quotation.QuotCode || `ID #${quotation.ID}`}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                  ...
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                    <span className="text-slate-500">Supplier Ref No.</span>
                    <span className="font-semibold text-slate-700">{quotation.CustRefNo || '—'}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                    <span className="text-slate-500">Posting Date</span>
                    <span className="font-semibold text-slate-700">{quotation.PostDate ? format(new Date(quotation.PostDate), 'yyyy-MM-dd') : '—'}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                    <span className="text-slate-500">Valid Until</span>
                    <span className="font-semibold text-slate-700">{quotation.DueDate ? format(new Date(quotation.DueDate), 'yyyy-MM-dd') : '—'}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                    <span className="text-slate-500">Department</span>
                    <span className="font-semibold text-slate-700">{quotation.Department || '—'}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                    <span className="text-slate-500">Created By</span>
                    <span className="font-semibold text-slate-700">{quotation.CreatedByName || quotation.CreatedBy || '—'}</span>
                  </div>
                  ...
                  {quotation.items?.map((item, idx) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-2.5 px-3 font-medium text-slate-600">{item.vendor || quotation.CustCode || '—'}</td>
              ...
              <div className="md:col-span-7 space-y-4">
                ...
                {quotation.attachments && quotation.attachments.length > 0 && (
                  <div className="space-y-2">
                    <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 block">Attached Files ({quotation.attachments.length})</span>
              ...
              <div className="md:col-span-5 bg-slate-50 p-5 rounded-2xl border border-slate-200/80 space-y-2.5 text-xs shadow-sm">
                ...
                {headerDisc > 0 && (
                  <div className="flex justify-between items-center text-slate-600">
                    ...
                    <span className="font-mono text-rose-600">-{currency} {discountAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                )}
                <div className="flex justify-between items-center text-slate-600">
                  <span>Total Before Tax</span>
                  <span className="font-mono font-semibold">{currency} {totalExclusive.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
                {Number(quotation.Freight || 0) > 0 && (
                  <div className="flex justify-between items-center text-slate-600">
                    <span>Freight Charges</span>
                    <span className="font-mono font-semibold">{currency} {Number(quotation.Freight || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                )}
                <div className="flex justify-between items-center text-slate-600">
                  <span>VAT Amount</span>
                  <span className="font-mono font-semibold">{currency} {Number(quotation.TaxTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
                ...
                <div className="flex justify-between items-center pt-3 mt-2 border-t border-slate-300">
                  ...
                  <span className="font-black font-mono text-emerald-700 text-lg">
                    {currency} {Number(quotation.DocTotal || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-10 border-t border-slate-200 grid grid-cols-3 gap-8 text-xs text-slate-600 avoid-page-break">
              <div>
                ...
                <p className="mt-1 text-[11px] text-slate-500">{quotation.CreatedByName || quotation.CreatedBy || 'Authorized User'}</p>
              </div>
              <div>
...
export default PurchaseQuotationReceiptView;
