...
export const ApproveSupplier: React.FC = () => {
  const [search, setSearch] = useState('');
  const [aprStatus, setAprStatus] = useState<string>('N');
  const [dateRange, setDateRange] = useState<DateRange>({ startDate: '', endDate: '' });
  const [selectedSupplierId, setSelectedSupplierId] = useState<number | null>(null);

  // React Query hooks for Pending Suppliers (CardType = 'S', AprStatus = 'N')
  const { data: pendingSuppliers = [], isLoading } = useRetailers({
    cardType: 'S',
    search,
    aprStatus,
