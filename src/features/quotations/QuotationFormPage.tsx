      ...
      if (existingQuotation.items && existingQuotation.items.length > 0) {
        setRows(
          existingQuotation.items.map(it => ({
            ...
            WhsCode: it.WhsCode ? Number(it.WhsCode) : 1,
            UoM: it.UoM || 'Pcs',
          }))
        );
      }
    ...
    <form onSubmit={handleSubmit} className="p-6 space-y-6 max-w-[1600px] mx-auto animate-fade-in pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Billing Address</label>
            <textarea
              value={address}
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Quotation Date *</label>
            <input
              type="date"
              value={postDate}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Valid Until / Due Date *</label>
            <input
              type="date"
              value={dueDate}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1">Customer Ref No.</label>
            <input
              type="text"
              value={custRefNo}
        ...
        <div className="bg-gradient-to-br from-teal-500/10 via-emerald-500/5 to-transparent p-6 rounded-2xl border border-teal-500/20 shadow-sm flex flex-col justify-between">
          ...
          <div className="pt-4 border-t border-teal-500/20 mt-4">
            <div className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Grand Total</div>
